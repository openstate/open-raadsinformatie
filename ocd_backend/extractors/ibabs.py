        ...
        for l in selected_lists:
            ...
            while (active_page_nr < max_pages) and (result_count == per_page):
                ...
                for item in results:
                    ...
                    if report_dict['datum'] and isinstance(report_dict['datum'], str) and (report_dict['datum'] not in ['G', 'V', 'R']) and not start_date < iso8601.parse_date(report_dict['datum']).replace(tzinfo=None) < end_date:
